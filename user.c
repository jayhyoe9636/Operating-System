...
int Spawn(const char *program, const char *command, struct Kernel_Thread **pThread)
{
    ...
    struct Exe_Format efm;
    char *efd = NULL;
    ulong_t efl;

    if (Read_Fully(program, (void **)&efd, &efl) != 0)
    {
        Print("Read_Fully error\n");
        return -1;
    }

    if (Parse_ELF_Executable(efd, efl, &efm) != 0)
    {
        Print("Parse_ELF_Executable error\n");
        return -1;
    }

    if (Load_User_Program(efd, efl, &efm, command, &uc) != 0)
    {
        Print("Load_User_Program error\n");
        return -1;
    }

    Free(efd);
    *pThread = Start_User_Thread(uc, false);
    if ((*pThread) == 0)
        return -1;
    else
        return (*pThread)->pid;
...
void Switch_To_User_Context(struct Kernel_Thread* kthread, struct Interrupt_State* state)
{
    ...
    struct User_Context *uc = kthread->userContext;
